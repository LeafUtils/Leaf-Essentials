var __typeError = (msg) => {
  ...
  var _storage, _PrismarineDBTable_instances, genID_fn, invokeEvent_fn, keyval_fn, updateEvent_fn, _ExtensionRegistry_instances, initialize_fn, _funcs, _groupName, _stores, _table2, _db, _defaultPermissions, _reservedTable, _reservedKeyVal, _reservedKeyVals, _reservedEconomyTable, _PrismarineDB_instances, getStorage_fn;
  ...
  keyval_fn = function(id2) {
    ...
    const has2 = (key) => {
      this.load();
      let doc2 = this.getByID(id2);
      if (doc2.data.__keyval_data[key]) {
        return true;
      } else {
        return false;
      }
  ...
  var PrismarineDB = class {
    ...
    scoreboardStorage(name) {
      if (name.startsWith("+PRISM:")) throw new Error("Database names starting with '+PRISM:' are reserved");
      return new PrismarineDBTable(name, new ScoreboardPersistentStorage());
    }
